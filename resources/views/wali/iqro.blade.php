...
<body class="antialiased min-h-screen flex flex-col pb-20 md:pb-0">
    ...
    <header class="bg-white/90 dark:bg-slate-950/90 backdrop-blur-md font-['Plus_Jakarta_Sans'] font-semibold text-lg docked full-width top-0 sticky border-b border-slate-100 dark:border-slate-800 shadow-sm shadow-emerald-900/5 transition-colors duration-200 ease-in-out flex justify-between items-center w-full px-4 h-16 z-50">
        <div class="flex items-center gap-4">
            ...
            <div class="flex flex-col">
                ...
                <span class="font-label-sm text-label-sm text-slate-500">Dashboard → Progres Iqro</span>
            </div>
        </div>
        <div class="flex items-center gap-4">
    ...
    <main class="flex-grow w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative">
        ...
        <div class="flex items-center gap-2 mb-8">
            ...
            <span class="text-on-surface font-medium font-label-md text-label-md">Progres Iqro</span>
        </div>
        ...
        @if($santri)
            ...
            <!-- Page Header -->
            <div class="mb-8">
                <h1 class="font-h2 text-h2 text-on-surface">📚 Progres Iqro</h1>
                <p class="font-body-md text-body-md text-on-surface-variant mt-1">Monitoring perkembangan belajar Iqro {{ $santri->nama_lengkap }}</p>
            </div>
                ...
                <section class="bg-primary-container rounded-2xl p-8 shadow-lg text-on-primary-container relative overflow-hidden mb-8">
                    ...
                    <div class="relative z-10">
                        ...
                        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                            <div class="flex items-center gap-4">
                                <div class="w-20 h-20 rounded-2xl bg-white/10 flex items-center justify-center text-4xl backdrop-blur-sm">
                                    {{ $latestIqro->status_lulus ? '✓' : '📖' }}
                                </div>
                                <div>
                                    <div class="font-h2 text-h2 text-white text-[28px]">{{ $latestIqro->level }} - Halaman {{ $latestIqro->halaman }}</div>
                                    <div class="flex items-center gap-4 mt-2 text-primary-fixed">
                                        <span class="flex items-center gap-1"><span class="material-symbols-outlined text-[18px]">calendar_today</span> {{ \Carbon\Carbon::parse($latestIqro->tanggal)->locale('id')->translatedFormat('d F Y') }}</span>
                                        <span class="flex items-center gap-1"><span class="material-symbols-outlined text-[18px]">person</span> {{ $latestIqro->guru->name ?? 'Guru' }}</span>
                                    </div>
                                </div>
                            </div>
                            <div class="flex-shrink-0">
                                <span class="inline-flex items-center rounded-full bg-white/20 px-4 py-2 font-label-md text-label-md font-semibold text-white backdrop-blur-sm">
                                    {{ $latestIqro->status_lulus ? 'LULUS ✨' : 'PROSES 📖' }}
                                </span>
                            </div>
                        </div>
                        @if($latestIqro->catatan_guru)
                            <div class="mt-6 bg-white/10 rounded-xl p-4 backdrop-blur-sm border border-white/10">
                                ...
                                <p class="font-body-md text-body-md text-primary-fixed">{{ $latestIqro->catatan_guru }}</p>
                            </div>
                        @endif
                    </div>
                </section>
            @else
                <div class="bg-surface-container-lowest rounded-xl p-8 shadow-sm border border-surface-variant mb-8 text-center">
                    <span class="material-symbols-outlined text-6xl text-on-surface-variant mb-4">menu_book</span>
                    <h3 class="font-h2 text-h2 text-on-surface mb-2">Belum Ada Progres</h3>
                    <p class="font-body-md text-body-md text-on-surface-variant">Belum ada data progres Iqro untuk {{ $santri->nama_lengkap }}</p>
                </div>
            @endif
            ...
            <section class="bg-surface-container-lowest rounded-xl border border-surface-variant shadow-[0_4px_20px_-4px_rgba(0,0,0,0.02)] overflow-hidden">
                <div class="p-6 border-b border-surface-variant bg-surface-container-low/50">
                    <h3 class="font-h3 text-h3 text-on-surface">📈 Riwayat Progres</h3>
                    <p class="font-body-sm text-body-sm text-on-surface-variant mt-1">Riwayat lengkap progres Iqro {{ $santri->nama_lengkap }}</p>
                </div>
                @if($riwayat->count() > 0)
                        ...
                        @foreach($riwayat as $item)
                            <div class="p-6 flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                                <div class="flex gap-4">
                                    ...
                                    <div>
                                        <h4 class="font-label-md text-label-md text-on-surface">{{ $item->level }} - Halaman {{ $item->halaman }}</h4>
                                        <p class="font-body-sm text-body-sm text-on-surface-variant mt-1">{{ \Carbon\Carbon::parse($item->tanggal)->locale('id')->translatedFormat('d M Y') }} • {{ $item->guru ? $item->guru->name : 'Guru' }}</p>
                                ...
                                <div class="flex-shrink-0">
                                    <span class="inline-flex items-center rounded-full {{ $item->status_lulus ? 'bg-primary-fixed text-on-primary-fixed-variant' : 'bg-surface-variant text-on-surface-variant' }} px-3 py-1.5 font-label-sm text-label-sm font-semibold">
                                        {{ $item->status_lulus ? 'LULUS' : 'PROSES' }}
                                    </span>
                                </div>
                            </div>
                        @endforeach
                ...
                @else
                    <div class="p-8 text-center">
                        ...
                        <p class="font-body-md text-body-md text-on-surface-variant">Belum ada riwayat progres Iqro untuk {{ $santri->nama_lengkap }}</p>
                    </div>
                @endif
            </section>
    ...
    <nav class="md:hidden bg-white dark:bg-slate-950 font-['Plus_Jakarta_Sans'] text-[11px] font-medium fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-4 pb-safe pt-2 h-20 rounded-t-2xl border-t border-slate-100 dark:border-slate-800 shadow-[0_-4px_12px_rgba(0,103,71,0.05)] transition-all duration-300 ease-in-out">
        ...
        <a class="flex flex-col items-center justify-center text-emerald-700 dark:text-emerald-400 font-bold bg-emerald-50/50 dark:bg-emerald-900/20 rounded-xl px-3 py-1 hover:text-emerald-600 dark:hover:text-emerald-300" href="{{ route('wali.iqro') }}">
            <span class="material-symbols-outlined mb-1 text-[24px]" data-icon="menu_book" data-weight="fill" style="font-variation-settings: 'FILL' 1;">menu_book</span>
            <span>Iqro</span>
        </a>
